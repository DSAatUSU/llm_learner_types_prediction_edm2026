...
EMB_N_COMPONENTS = 16          # per embedding column (code + task)
IPCA_BATCH_SIZE  = 1024        # adjust if memory issues

# Sequence padding
MAXLEN_PERCENTILE = 90         # use 90th percentile of TRAIN sequence lengths
MIN_MAXLEN        = 2
...
# Feature filtering / encoding
NUM_MISSINGNESS_THRESH = 0.25
CAT_MISSINGNESS_THRESH = 0.25
MIN_VARIANCE = 1e-4
MIN_CAT_CARDINALITY = 2
...
train_seq_meta = seq_meta.loc[seq_meta["user_id"].isin(train_students)].copy()
test_seq_meta  = seq_meta.loc[seq_meta["user_id"].isin(test_students)].copy()

if USE_GROUP_VAL_SPLIT:
    gss = GroupShuffleSplit(n_splits=1, test_size=VAL_SIZE, random_state=SEED)
    train_idx, val_idx = next(gss.split(train_seq_meta, groups=train_seq_meta["user_id"]))
    train_seq_meta_ = train_seq_meta.iloc[train_idx]
    val_seq_meta_ = train_seq_meta.iloc[val_idx]

    # Ensure all classes remain in train; fallback if needed.
    if set(train_seq_meta_["label"]) != set(train_seq_meta["label"]):
        print("Group val split dropped some classes; falling back to stratified split.")
        sss = StratifiedShuffleSplit(n_splits=1, test_size=VAL_SIZE, random_state=SEED)
        train_idx, val_idx = next(sss.split(train_seq_meta, train_seq_meta["label"]))
        train_seq_meta_ = train_seq_meta.iloc[train_idx]
        val_seq_meta_ = train_seq_meta.iloc[val_idx]
else:
    sss = StratifiedShuffleSplit(n_splits=1, test_size=VAL_SIZE, random_state=SEED)
...
train_seq_ids = train_seq_meta_["sequence_id"].values
val_seq_ids   = val_seq_meta_["sequence_id"].values
test_seq_ids  = test_seq_meta["sequence_id"].values

print(f"Train sequences: {len(train_seq_ids)} | Val sequences: {len(val_seq_ids)} | Test sequences: {len(test_seq_ids)}")


# Row indices for fitting snapshot-level preprocessors (PCA/scaler) on TRAIN rows only
train_row_mask = df["sequence_id"].isin(train_seq_ids).to_numpy()
...
# -----------------------------
# Build feature lists + filter (train-only stats)
# -----------------------------
...
AUTO_CATEGORICAL_EXCLUDE = {"user_id", "exercise_id", "sequence_id", "label"} | set(EMB_COLS)
auto_cat = [
...
# -----------------------------
# Choose available snapshot features (filter with TRAIN stats)
# -----------------------------
available_numeric_features = [f for f in numeric_features if f in df.columns and f not in AUTO_CATEGORICAL_EXCLUDE]
...
# Filter numeric by missingness + variance (train-only)
dropped_missing = []
dropped_lowvar = []
kept_numeric = []
for f in available_numeric_features:
    miss = df.loc[train_row_mask, f].isna().mean()
    var = df.loc[train_row_mask, f].var(skipna=True)
    if miss > NUM_MISSINGNESS_THRESH:
        dropped_missing.append((f, float(miss)))
        continue
    if var < MIN_VARIANCE:
        dropped_lowvar.append((f, float(var)))
        continue
    kept_numeric.append(f)

available_numeric_features = kept_numeric
print(f"Numeric kept after filtering: {len(available_numeric_features)}")
if dropped_missing:
    dropped_missing = sorted(dropped_missing, key=lambda x: x[1], reverse=True)
    print(f"Dropped numeric (missingness > {NUM_MISSINGNESS_THRESH:.2f}): {len(dropped_missing)}")
    print("  Examples:", [f for f, _ in dropped_missing[:10]])
if dropped_lowvar:
    dropped_lowvar = sorted(dropped_lowvar, key=lambda x: x[1])
    print(f"Dropped numeric (variance < {MIN_VARIANCE:g}): {len(dropped_lowvar)}")
    print("  Examples:", [f for f, _ in dropped_lowvar[:10]])

# Impute numeric using TRAIN means only
numeric_impute_values = {}
for f in available_numeric_features:
    m = df.loc[train_row_mask, f].mean()
    if pd.isna(m):
        m = 0.0
    df[f] = df[f].fillna(m)
    numeric_impute_values[f] = float(m)

# Filter categorical by missingness + cardinality (train-only)
categorical_features = [c for c in categorical_features if c in df.columns and c not in AUTO_CATEGORICAL_EXCLUDE]
cat_dropped_missing = []
cat_dropped_card = []
cat_dropped_unhashable = []
cat_keep = []
for c in categorical_features:
    series_train = df.loc[train_row_mask, c]
    # Drop columns that contain unhashable array-like objects (e.g., embeddings)
    if series_train.apply(lambda v: isinstance(v, (list, np.ndarray, dict))).any():
        cat_dropped_unhashable.append(c)
        continue

    miss = series_train.isna().mean()
    try:
        nunique = series_train.nunique(dropna=True)
    except TypeError:
        cat_dropped_unhashable.append(c)
        continue
    if miss > CAT_MISSINGNESS_THRESH:
        cat_dropped_missing.append((c, float(miss)))
        continue
    if nunique < MIN_CAT_CARDINALITY or nunique > MAX_CAT_CARDINALITY:
        cat_dropped_card.append((c, int(nunique)))
        continue
    cat_keep.append(c)

categorical_features = cat_keep
print(f"Categorical kept after filtering: {len(categorical_features)}")
if cat_dropped_missing:
    cat_dropped_missing = sorted(cat_dropped_missing, key=lambda x: x[1], reverse=True)
    print(f"Dropped categorical (missingness > {CAT_MISSINGNESS_THRESH:.2f}): {len(cat_dropped_missing)}")
    print("  Examples:", [c for c, _ in cat_dropped_missing[:10]])
if cat_dropped_card:
    cat_dropped_card = sorted(cat_dropped_card, key=lambda x: x[1], reverse=True)
    print(f"Dropped categorical (cardinality outside [{MIN_CAT_CARDINALITY}, {MAX_CAT_CARDINALITY}]): {len(cat_dropped_card)}")
    print("  Examples:", [c for c, _ in cat_dropped_card[:10]])
if cat_dropped_unhashable:
    print(f"Dropped categorical (unhashable array-like values): {len(cat_dropped_unhashable)}")
    print("  Examples:", cat_dropped_unhashable[:10])
...
print(f" ALL FEATURES USED: {available_numeric_features + cat_feature_names}")
...
# -----------------------------
# Build sequential model (smaller + stronger regularization)
# -----------------------------
...
x = layers.Bidirectional(
    layers.GRU(64, return_sequences=True, dropout=0.2, recurrent_dropout=RECURRENT_DROPOUT, kernel_regularizer=reg)
)(x)
x = layers.LayerNormalization()(x)

x = layers.Bidirectional(
    layers.GRU(32, return_sequences=True, dropout=0.2, recurrent_dropout=RECURRENT_DROPOUT, kernel_regularizer=reg)
)(x)
...
try:
    ...
    print("Label smoothing not supported in this TF version; disabling.")
...
preproc = {
    ...
    "numeric_dropped_missing": dropped_missing,
    "numeric_dropped_lowvar": dropped_lowvar,
    "categorical_dropped_missing": cat_dropped_missing,
    "categorical_dropped_cardinality": cat_dropped_card,
    "categorical_dropped_unhashable": cat_dropped_unhashable,
}
